...
const fallbackUpdates = [
  {
    id: "fallback-20260819",
    date: "2026.08.19",
    title: "알파벳 빙고 무작위 배치가 더 편해졌어요",
    items: [
      "알파벳 빙고에서 무작위 배치를 선택하면 방을 만들기 전에 실제 배치를 미리 확인할 수 있습니다.",
      "다시 셔플 버튼을 눌러 원하는 배치가 나올 때까지 알파벳 순서를 바꿀 수 있습니다.",
      "빙고방을 생성하면 미리보기에서 확인한 알파벳 배치가 그대로 적용됩니다.",
      "빙고판 크기를 변경하면 선택한 크기에 맞춰 새로운 무작위 배치 미리보기가 자동으로 만들어집니다."
    ],
    published: true,
    fallback: true
  },
  {
    id: "fallback-20260814",
    date: "2026.08.14",
    title: "화면과 이용 편의성이 더 좋아졌어요",
    items: [
      "한 번 로그인하면 브라우저를 다시 열어도 로그인 상태가 더 안정적으로 유지됩니다.",
      "상단 메뉴를 간결하게 정리해 홈, 빙고, 업데이트, 채널 이동과 내 정보를 더 쉽게 찾을 수 있습니다.",
      "상단 메뉴에서 채널 프로필 사진과 Google 계정 프로필 사진을 함께 확인할 수 있습니다.",
      "자주 이용하는 채널을 즐겨찾기에 등록해 채널 선택 화면에서 빠르게 들어갈 수 있습니다.",
      "모바일에서도 상단 메뉴가 길어지지 않도록 화면 구성을 개선했습니다.",
      "내 정보, 채널 정보와 채널 공유 화면의 디자인을 개선해 입력 항목과 버튼을 더 편하게 확인할 수 있습니다.",
      "삭제된 채널이 채널 검색 결과에 남아 보이는 문제를 개선했습니다.",
      "가입한 채널로 이동하는 버튼 문구를 더 알아보기 쉬운 '채널 입장'으로 변경했습니다.",
      "채널을 채널명이 포함된 공유 링크로 간편하게 초대할 수 있습니다.",
      "사용자명을 직접 변경할 수 있으며 이미 사용 중인 사용자명은 사용할 수 없습니다.",
      "채널 소유자는 채널 이름과 프로필 사진을 직접 변경할 수 있으며 이미 사용 중인 채널 이름은 사용할 수 없습니다.",
      "채널 이용이 중지되거나 권한이 변경되면 현재 화면에도 빠르게 반영됩니다.",
      "사용자명이나 채널 정보를 저장하면 새로고침 없이 변경 내용이 바로 화면에 반영됩니다.",
      "채널 이용이 중지되어도 기존 빙고방과 이용 기록은 삭제되지 않습니다."
    ],
    published: true,
    fallback: true
  },
  {
    id: "fallback-20260813",
    date: "2026.08.13",
    title: "채널 이용과 사용자 관리가 개선됐어요",
    items: [
      "채널 가입 신청은 채널 소유자 또는 관리자의 승인 후 이용할 수 있습니다.",
      "채널 사용자 관리에서 가입 승인과 사용 중인 사용자를 확인할 수 있습니다.",
      "채널에서 추방되는 사용자가 진행 중인 빙고방의 방장인 경우 해당 방은 자동으로 종료됩니다.",
      "채널마다 빙고·킬내기 중 제공되는 기능이 다를 수 있으며 제공되는 메뉴만 이용할 수 있습니다."
    ],
    published: true,
    fallback: true
  },
  {
    id: "fallback-20260812",
    date: "2026.08.12",
    title: "빙고 기능과 편의성이 더 좋아졌어요",
    items: [
      "빙고판 전체 선택·전체 해제와 실행 취소를 사용할 수 있습니다.",
      "체크된 칸, 남은 칸, 완성 빙고 수와 먹은 치킨 수량을 한눈에 확인할 수 있습니다.",
      "치킨 수량 변경 기록에서 변경 사용자와 시간을 확인하고 잘못된 기록을 취소할 수 있습니다.",
      "숫자 빙고 외에 알파벳 빙고와 자유 텍스트 빙고를 만들 수 있습니다.",
      "알파벳 직접 지정, 한번에 입력, 자동 채우기와 섞기 기능을 사용할 수 있습니다.",
      "빙고방 종료 결과 보관, 방장 위임, 방 복제, 초대 링크·QR 코드를 사용할 수 있습니다.",
      "방 목록 검색·필터·즐겨찾기와 네트워크 상태 안내가 추가되었습니다."
    ],
    published: true,
    fallback: true
  },
  ...
  try {
    if (editingUpdateId) {
      ...
      if (fallbackIds.has(editingUpdateId)) {
        ...
        if (alreadyOverridden) await updateDoc(doc(db, "platformUpdates", editingUpdateId), { ...payload, published: true });
        else await setDoc(doc(db, "platformUpdates", editingUpdateId), { ...payload, published: true, createdAt: serverTimestamp() });
      } else {
        await updateDoc(doc(db, "platformUpdates", editingUpdateId), payload);
      }
      ...
      await addDoc(collection(db, "platformUpdates"), { ...payload, createdAt: serverTimestamp() });
...
async function removeManagedUpdate(update) {
  ...
  try {
    await deleteDoc(doc(db, "platformUpdates", update.id));
...
async function loadUpdates() {
  ...
  try {
    const ref = collection(db, "platformUpdates");
